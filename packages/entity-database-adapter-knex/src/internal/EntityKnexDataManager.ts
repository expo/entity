...
> {
  ...
  async loadPageAsync(
    ...
    if (pagination.strategy === PaginationStrategy.STANDARD) {
      ...
      const whereClause = [where, searchWhere]
        .filter((it) => !!it)
        .reduce<SQLFragment | undefined>((acc, condition) => {
          if (!acc) {
            return condition;
          }
          return SQLFragmentHelpers.and(acc, condition);
        }, undefined);
  ...
  private buildTrigramCursorCondition(
    ...
    const exactMatchExpr = this.buildTrigramExactMatchCaseExpression(search, direction);
  ...
  private buildTrigramExactMatchPriority(
    ...
    // For forward pagination: exact matches get priority 1, others get 0
    // For backward pagination: reverse the values for proper cursor comparison
    const exactMatchPriorityValue = direction === PaginationDirection.FORWARD ? '1' : '0';
    const nonExactMatchPriorityValue = direction === PaginationDirection.FORWARD ? '0' : '1';
    ...
    return sql`CASE WHEN ${exactMatchCondition} THEN ${raw(exactMatchPriorityValue)} ELSE ${raw(nonExactMatchPriorityValue)} END ${raw(sortOrder)}`;
  }
