...
> extends EntityDatabaseAdapter<TFields> {
  ...
  protected async fetchManyWhereInternalAsync(
    ...
    // postgres type casting on each value in each tableValueTuple, thus creating a unique query shape and defeating the purpose.
    ...
    if (tableColumnTuple.length === 1) {
      return await wrapNativePostgresCallAsync(() =>
        queryInterface
          ...
          .whereRaw(`(??) = ANY(?)`, [
            tableColumnTuple[0],
            tableValueTuples.map((tableValueTuple) => tableValueTuple[0]),
          ]),
      );
    }

    return await wrapNativePostgresCallAsync(() =>
      queryInterface.select().from(tableName).whereIn(tableColumnTuple, tableValueTuples),
    );
  }
